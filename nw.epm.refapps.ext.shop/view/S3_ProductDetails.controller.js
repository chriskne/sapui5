...
sap.ui.core.mvc.Controller.extend("nw.epm.refapps.ext.shop.view.S3_ProductDetails", {
    ...
    _addFavoriteButtonText : function() {
        ...
        var oBinding = this.byId("productDetailsPage").getElementBinding(), fnChangeHandler = null, sFavorite;
        fnChangeHandler = jQuery.proxy(function() {
            if (this.byId("productDetailsPage").getBindingContext().getProperty("IsFavoriteOfCurrentUser")) {
                ...
                this.byId("btnFavorite").setText(sFavorite);
            } else {
    ...
    _onRoutePatternMatched : function(oEvent) {
        ...
        this._sProductPath = "/Products('" + decodeURIComponent(oEvent.getParameter("arguments").productId) + "')";
    ...
    onRatingPressed : function(oEvent) {
        ...
        if (sap.ui.Device.system.phone) {
            ...
            var oRatingCount = this.getView().byId("ratingCount"), position = oRatingCount.$().offset();
    ...
    onToggleFavoritePressed : function() {
        var oBindingContext = this.byId("productDetailsPage").getBindingContext(), sFavorite;
        var oData = {};
        oData.IsFavoriteOfCurrentUser = !(oBindingContext.getProperty("IsFavoriteOfCurrentUser"));

        this._oComponent.getModel().update(oBindingContext.getPath(), oData, {
            merge : true,
            success : null,
            error : jQuery.proxy(this.onSrvError, this)
        });
        ...
        if (!sap.ui.Device.system.desktop) {
            if (oData.IsFavoriteOfCurrentUser) {
                sFavorite = this._oResourceBundle.getText("xbut.removeFavorite");
                this.byId("btnFavorite").setText(sFavorite);
            } else {
                sFavorite = this._oResourceBundle.getText("xbut.addFavorite");
    ...
    onEditReviewPressed : function() {
        ...
        var bHasOwnReview = false, oBTNDelete = sap.ui.getCore().byId("reviewDelete", "reviewDialog");
        ...
        for (i = 0; i < this._oReviewTable.getItems().length; i++) {
            if (this._oReviewTable.getItems()[i].getBindingContext().getProperty("IsReviewOfCurrentUser")) {
                ...
                this._oReviewDialog.bindElement(this._oReviewTable.getItems()[i].getBindingContext().getPath());
                bHasOwnReview = true;
                oBTNDelete.setVisible(bHasOwnReview);
                break;
            }
    ...
    _toggleReviewButtonText : function() {
        var oBinding = this.byId("productDetailsPage").getElementBinding(), fnChangeHandler = null;
        fnChangeHandler = jQuery.proxy(function() {
    ...
    onRateAsHelpfulPressed : function(oEvent) {
        var oData = {};
        oData.HelpfulForCurrentUser = true;

        // Use MERGE to update only the changed property and not all properties
        this._oComponent.getModel().update(oEvent.getSource().getBindingContext().getPath(), oData, {
            merge : true,
            error : jQuery.proxy(this.onSrvError, this)
        });
    },
    ...
    onReviewDialogOKPressed : function(oEvent) {
        ...
        var oData = {};
        oData.Comment = sap.ui.getCore().byId("textArea", "reviewDialog").getValue();
        oData.Rating = sap.ui.getCore().byId("ratingIndicator", "reviewDialog").getValue();
        oData.ProductId = this.byId("productId").getText();

        if (!this._bIsEditReview) {
            oEvent.getSource().getModel().create(this._sReviewPath, oData, {
                success : jQuery.proxy(this.onReviewSrvSuccess, this),
                error : jQuery.proxy(this.onSrvError, this)
            });
        } else {
            oEvent.getSource().getModel().update(oEvent.getSource().getBindingContext().getPath(), oData, {
                success : jQuery.proxy(this.onReviewSrvSuccess, this),
                error : jQuery.proxy(this.onSrvError, this)
            });
        }
    ...
    onRatingChanged : function() {
        if (sap.ui.getCore().byId("textArea", "reviewDialog").getValue()) {
            sap.ui.getCore().byId("btnOK", "reviewDialog").setEnabled(true);
        }
    },
